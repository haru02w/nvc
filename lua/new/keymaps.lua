...
keymap('n', "<leader>w", ":noautocmd w<CR>", { desc = 'Save without formatting' })
-- Quick Split
...
keymap('x', 'p', '\"_dP', { desc = 'a better paste' })
...
function M.lsp()
	...
	keymap('n', '<leader>ld', vim.diagnostic.open_float, { desc = "Open floating diagnostic message" })
	keymap('n', '<leader>ll', vim.diagnostic.setloclist, { desc = "Open diagnostics list" })
	keymap('n', '<leader>lr', vim.lsp.buf.rename, { desc = '[L]sp [R]ename' })
	keymap({ 'n', 'v' }, '<leader>la', vim.lsp.buf.code_action, { desc = '[L]sp Code [A]ction' })
...
function M.bbye()
	keymap('n', '<leader>c', vim.cmd.Bdelete, { desc = 'close buffer' })
end
